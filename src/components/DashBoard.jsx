...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [userDetails, stats, monthlyStats, topicStats] = await Promise.all([
          fetch('https://mernede.vercel.app/userdetails', { headers: { 'Authorization': `Bearer ${authToken}` } }).then(res => res.json()),
  ...
  const handleFormSubmit = async (formType, formData) => {
    try {
      ...
      switch (formType) {
        case 'profile':
          response = await fetch('https://mernede.vercel.app/user/profile', {
            method: 'PUT',
            headers: {
          ...
          break;
        case 'password':
          response = await fetch('https://mernede.vercel.app/user/change-password', {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${authToken}`
          ...
          break;
        case 'delete':
          response = await fetch('https://mernede.vercel.app/user/account', {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${authToken}`
            }
