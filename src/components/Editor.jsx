...
const Editor = () => {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        const response = await axios.get(`https://mernede.vercel.app/problemdetails/${problemId}`);
  ...
  const handleRun = async () => {
    try {
      ...
      const response = await axios.post('https://onlinejudge-2nas.onrender.com/run', {
        language,
        code,
        input: customInput
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      setTestResults([]);
      const response = await fetch(`https://onlinejudge-2nas.onrender.com/submit/${problemId}`,  {
        method: 'POST',
        headers: {
